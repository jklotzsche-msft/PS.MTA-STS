...
Copy-Item -Path "$($WorkingDirectory)\PS.MTA-STS" -Destination $publishDir.FullName -Recurse -Force
...
[System.IO.File]::WriteAllText("$($publishDir.FullName)\PS.MTA-STS\PS.MTA-STS.psm1", ($text -join "`n`n"), [System.Text.Encoding]::UTF8)
Remove-Item -Path "$($publishDir.FullName)\PS.MTA-STS\internal" -Recurse -Force
Remove-Item -Path "$($publishDir.FullName)\PS.MTA-STS\functions" -Recurse -Force
